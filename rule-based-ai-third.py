...
def agent(obs, config):
    ...
    if turn == 0:
        ...
        for cell in halite_spots:
            for ship in me.ships:
                options += [(cell.halite*.25 - distance(ship.position,
                                                        cell.position)*10, cell.position, ship, False, False)]

        opponent_shipyards = list(filter(
            lambda kami: kami.player.id != me.id, board.shipyards.values()))
        for ship in me.ships:
            if ship.halite == 0:
                for kami in opponent_shipyards:
                    options += [(325*0.25 - distance(ship.position,
                                                     kami.position)*10, kami.position, ship, False, True)]
            ...
            for ship in me.ships:
                ...
                for kami in me.shipyards:
                    options += [(ship.halite*0.5 - distance(ship.position,
                                                            kami.position)*10, kami.position, ship, True, False)]
        ...
        for option in options:
            ...
            allow_crash = option[4]
            next_action = (None, ship.position)
            if ship.position != position:
                next_action = move_to(
                    ship.position, position)
            next_position = next_action[1]
            if ship.id not in moved and position not in assigned and next_position not in moved_to and (allow_crash or (ship.halite < danger[next_position] if next_position in danger else True)):
                moved_to.add(next_position)
                moved[ship.id] = next_position
